	...
	void LateUpdate () {
		...
		if (particlesAlive > 0) 
		{
			if (oneLightOnly) //When usign just one light
			{
				...
				float normalizedLifetime = 1 - particles[particleToSample].remainingLifetime/particles[particleToSample].startLifetime;  //Get the current life time normalized 
			...
			else //When n lights
			{
				...
				{
					float normalizedLifetime = 1 - particles[i].remainingLifetime/particles[i].startLifetime; //Get the current life time normalized 
